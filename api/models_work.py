...
@receiver(m2m_changed, sender=RustDeskPeer.tags.through)
def validate_peer_tag_profile(sender, instance, action, reverse, pk_set, **kwargs):
    ...
    if action != 'pre_add' or not pk_set:
        return
    if reverse:
        has_mismatch = RustDeskPeer.objects.filter(pk__in=pk_set).exclude(
            profile_id=instance.profile_id,
        ).exists()
    else:
        has_mismatch = RustDeskTag.objects.filter(pk__in=pk_set).exclude(
            profile_id=instance.profile_id,
        ).exists()
    if has_mismatch:
...
class RustDeskDevice(models.Model):
...
class RustDeskDeviceAdmin(admin.ModelAdmin):
    list_display = ('rid', 'hostname', 'memory', 'uuid', 'version', 'owner', 'device_group', 'strategy', 'create_time', 'update_time')
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                condition=models.Q(conn_end__isnull=True)
                | models.Q(conn_end__gte=models.F('conn_start')),
            ...
            models.CheckConstraint(
                condition=models.Q(primary_auth__isnull=True)
                | models.Q(primary_auth__gte=1, primary_auth__lte=4),
                name='valid_primary_auth_method',
            ),
            models.CheckConstraint(
                condition=models.Q(two_factor__isnull=True)
                | models.Q(two_factor__gte=1, two_factor__lte=2),
                name='valid_second_factor_method',
            ),
            models.CheckConstraint(
                condition=models.Q(conn_type__isnull=True)
                | models.Q(conn_type__gte=0, conn_type__lte=4),
...
def validate_share_link_peer_owner(
    ...
    **kwargs,
    ...
    if action != 'pre_add' or not pk_set:
        return
    if reverse:
        has_mismatch = ShareLink.objects.filter(pk__in=pk_set).exclude(
...
class StrategyProfile(models.Model):
    ...
    def clean(self):
        super().clean()
        if (
            not isinstance(self.config_options, dict)
            or any(
