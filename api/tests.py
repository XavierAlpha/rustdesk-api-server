...
class ApiTestMixin:
    ...
    def _device(self, owner=None, rid="123456789", uuid=DEFAULT_DEVICE_UUID, **overrides):
        ...
        return RustDeskDevice.objects.create(**data)
...
class ApiContractTests(ApiTestMixin, TestCase):
    def test_login_requires_bearer_token_for_current_user(self):
        ...
        device = RustDeskDevice.objects.get(rid="123456789")
        self.assertEqual(device.hostname, "desktop")
        self.assertEqual(device.os, "linux")
        stored_token = RustDeskToken.objects.get(device=device)
    ...
    def test_login_rejects_noncanonical_device_identity(self):
        ...
        self.assertFalse(RustDeskToken.objects.exists())
    ...
    def test_successful_login_clears_failures_and_stores_hashed_token(self):
        ...
        stored = RustDeskToken.objects.get(device__owner=self.user)
    ...
    def test_telemetry_requires_the_token_bound_to_that_device(self):
        ...
        self.assertFalse(RustDeskDevice.objects.exists())
        ...
        self.assertTrue(RustDeskDevice.objects.filter(rid="123456789").exists())
    ...
    def test_address_book_tag_relations_survive_merge_and_reject_cross_book_links(self):
        ...
        old_tag = RustDeskTag.objects.create(
            profile=profile,
            tag_name="old",
        )
        target_tag = RustDeskTag.objects.create(
            profile=profile,
            tag_name="target",
        )
        ...
        self.assertFalse(
            RustDeskTag.objects.filter(profile=profile, tag_name="old").exists()
        )
        self.assertEqual(
        ...
        foreign_tag = RustDeskTag.objects.create(
        ...
        self.assertFalse(RustDeskTag.objects.filter(pk=target_tag.pk).exists())

    def test_address_book_credentials_are_authenticated_encrypted_at_rest(self):
        profile = self._personal_profile(self.user)
        first = RustDeskPeer.objects.create(
            profile=profile,
            rid="765432100",
            rhash="same-sensitive-value",
            password="same-sensitive-value",
        )
        second = RustDeskPeer.objects.create(
            profile=profile,
            rid="765432101",
            rhash="same-sensitive-value",
        )
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT rhash, password FROM api_rustdeskpeer WHERE id = %s",
                [first.pk],
            )
            raw_rhash, raw_password = cursor.fetchone()
            cursor.execute(
                "SELECT rhash FROM api_rustdeskpeer WHERE id = %s",
                [second.pk],
            )
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                "UPDATE api_rustdeskpeer SET rhash = %s WHERE id = %s",
    ...
    def test_admin_can_manage_users_and_disabled_users_cannot_login(self):
        ...
        self.assertEqual(
            list(
                UserProfile.objects.get(username="bob")
    ...
    def test_accessible_inventory_never_joins_an_unowned_device_by_id(self):
        ...
        own_device = RustDeskDevice.objects.get(rid="123456789")
        own_device.device_group = own_group
        own_device.save(update_fields=["device_group"])
        RustDeskDevice.objects.filter(pk=own_device.pk).update(
        ...
        self.assertEqual(peers.status_code, 200, peers.content)
        peer_items = {
            item["id"]: item for item in peers.json()["data"]
        }
        self.assertEqual(
            set(peer_items),
            {"123456789", "765432100"},
    ...
    def test_device_deployment_is_bound_to_uuid_key_and_active_owner(self):
        ...
        self.assertFalse(RustDeskToken.objects.filter(access_token=hashlib.sha256(token.encode()).hexdigest()).exists())
    ...
    def test_inactive_owner_invalidates_device_heartbeat(self):
        ...
        self.assertFalse(
            RustDeskToken.objects.filter(device__owner=self.user).exists()
        )
    ...
    def test_front_device_pages_merge_legacy_metadata_and_expose_unknown_state(self):
        ...
        personal_profile = self._personal_profile(self.user)
        work_tag = RustDeskTag.objects.create(
            profile=personal_profile,
            tag_name="work",
        )
        mobile_tag = RustDeskTag.objects.create(
            profile=personal_profile,
            tag_name="mobile",
        )
        primary_peer = RustDeskPeer.objects.create(
            profile=personal_profile,
            rid="765432100",
            username="desktop-user",
            hostname="desktop",
        ...
        address_book_peer = RustDeskPeer.objects.create(
            profile=personal_profile,
            rid="765432101",
            username="address-book-user",
            hostname="unknown",
    ...
    def test_share_link_is_hashed_preview_only_and_single_use(self):
        ...
        source_tag = RustDeskTag.objects.create(
            profile=profile,
            tag_name="studio",
        )
        ...
        imported = RustDeskPeer.objects.get(
            profile__owner=recipient,
            rid=peer.rid,
        )
...
class SessionIntegrityTests(ApiTestMixin, TestCase):
    ...
    def test_logout_revokes_the_device_that_asked(self):
        ...
        self.assertEqual(
            RustDeskToken.objects.filter(device__owner=self.user).count(),
            2,
        )
