...
def _project_version(root: Path) -> str:
    ...
    if not isinstance(project, dict) or project.get("name") != "camellia-api-server":
        raise MetadataError("pyproject.toml must describe project camellia-api-server")
...
def _lock_version(root: Path) -> str:
    ...
    matches = [
        ...
        and package.get("name") == "camellia-api-server"
        and package.get("source") == {"virtual": "."}
    ]
    if len(matches) != 1 or not isinstance(matches[0].get("version"), str):
        raise MetadataError("uv.lock must contain exactly one virtual camellia-api-server package")
...
def load_metadata(root: Path) -> ReleaseMetadata:
    ...
    if lock_version != project_version or legacy_version != expected_legacy:
        raise MetadataError(
            "version mismatch: "
            f"pyproject.toml={project_version}, uv.lock={lock_version}, version.py={legacy_version}"
        )
